 ...
 * 
 * 报文头 + 数据长度 + 数据 + 校验
 * byte[2] + byte[2] + bytes + byte[16]
 */
public class SampleChatCheckDecoder extends ByteToMessageDecoder {

    private static final Logger logger = LoggerFactory.getLogger(SampleChatCheckDecoder.class);

    private static final byte[] HEADER = { 0x75, 0x72 }; // 报文头

    private static final int MIN_LENGTH = 2 + 2 + 16; // 报文头 + 数据长度 + md5
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        // 验证签名
        if (!Arrays.equals(DigestUtils.md5(datas), sign)) {
            logger.debug("签名认证失败");
            return;
        }
